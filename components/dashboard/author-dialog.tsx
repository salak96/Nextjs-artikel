...
interface Props {
  edit?: { id: string; name: string; email: string; title: string | null; image: string | null };
}

export function AuthorDialog({ edit }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/paneladmin/authors", {
        method: edit ? "PUT" : "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(edit ? { id: edit.id, name, email, title: title || null } : { name, email, title: title || null }),
      });
      if (res.ok) {
        setOpen(false);
        router.refresh();
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{edit ? "Edit Author" : "New Author"}</DialogTitle>
        </DialogHeader>
